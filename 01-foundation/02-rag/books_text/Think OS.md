...
TheLATEXsourceforthisbookisavailablefromhttp://greenteapress.com/
thinkos.
...
goalsistohelpthemlearnC.Forthatpartoftheclass, IuseGriffithsandGrif-
...
| ---------- | --------- | --- | --- | --- |
Chapter2explains how theoperatingsystem usesprocessesto protectrunning
...
Chapter9isaboutPOSIXthreadsandmutexes. Recommendedreading: Head
...
|     | to  | your | computer. |     |     |     |     |     |     |     |
| --- | --- | ---- | --------- | --- | --- | --- | --- | --- | --- | --- |
(cid:136) Or you could clone my repository. You don’t need a GitHub account to
...
| --- | ---- | ------- | ----- | ------- | --- | --- | --- | --- | --- | --- |
(cid:136) I am grateful to the students in Software Systems at Olin College, who tested
...
| --- | --------------- | ---- | ---------- | --- | ------------ | ---------------- | --- | ---------------- | ------- | --- |
|     | (cid:136) James | P    | Giannoules |     | spotted      | a copy-and-paste |     | error.           |         |     |
(cid:136)
...
gram is running. In general, static refers to things that happen at compile
time (while a program is being compiled), and dynamic refers to things that
...
| other types | will  | cause | an     | exception | or       | runtime | error. |     |
| ----------- | ----- | ----- | ------ | --------- | -------- | ------- | ------ | --- |
...
| 1.3. | The | compilation |     |     | process |     |     | 3   |
| ---- | --- | ----------- | --- | --- | ------- | --- | --- | --- |
Thisshowsthatthenameofthevariableisstoredinmemorywhiletheprogram
...
1. Preprocessing: Cisoneofseverallanguagesthatincludepreprocessing
directivesthattakeeffectbeforetheprogramiscompiled. Forexample,
...
code.
|     | 1This | is a simplification; |     | we  | will go | into more | detail later. |     |
| --- | ----- | -------------------- | --- | --- | ------- | --------- | ------------- | --- |

| 4   |     |     |     |     | Chapter | 1. Compilation |
| --- | --- | --- | --- | --- | ------- | -------------- |
6. Optimization: At several points in the process, the compiler can trans-
...
Normally when you run gcc, it runs all of these steps and generates an exe-
...
| 1.4 | Object | code |     |     |     |     |
| --- | ------ | ---- | --- | --- | --- | --- |
The -c flag tells gcc to compile the program and generate machine code, but
| not to link | it or generate | an  | executable: |     |     |     |
| ----------- | -------------- | --- | ----------- | --- | --- | --- |
| $ gcc       | hello.c        | -c  |             |     |     |     |
The result is a file named hello.o, where the o stands for object code, which
is the compiled program. Object code is not executable, but it can be linked
into an executable.
The UNIX command reads an object file and generates information about
nm
...
| 1.5. Assembly | code |     |     |     | 5   |
| ------------- | ---- | --- | --- | --- | --- |
This output indicates that hello.o defines the name main and uses a function
named puts, which stands for “put string”. In this example, gcc performs an
optimization by replacing printf, which is a large and complicated function,
| with puts, | which is relatively | simple. |     |     |     |
| ---------- | ------------------- | ------- | --- | --- | --- |
You can control how much optimization gcc does with the -O flag. By default,
it does very little optimization, which can make debugging easier. The option
-O1 turns on the most common and safe optimizations. Higher numbers turn
...
| ------- | -------------------- | ----- | --- | --- | --- |
| 1.5     | Assembly             | code  |     |     |     |
Similartothe-cflag, the-Sflagtellsgcctocompiletheprogramandgenerate
...
the included code from stdio.h, and all the files included from stdio.h, and
all the files included from those files, and so on. On my machine, the total is
more than 800 lines of code. Since almost every C program includes stdio.h,
those 800 lines of code get compiled a lot. If, like many C programs, you also
| include | stdlib.h,     |     | the result | is  | more than | 1800 | lines of | code. |     |
| ------- | ------------- | --- | ---------- | --- | --------- | ---- | -------- | ----- | --- |
| 1.7     | Understanding |     |            |     | errors    |      |          |       |     |
Now that we know the steps in the compilation process, it is easier to under-
stand error messages. For example, if there is an error in a #include directive,
...
(cid:136) Abstraction: An abstraction is a simplified representation of something
...
tion.
(cid:136) Virtualization: An important kind of abstraction is virtualization, which
...
| packetsfromonenetworktothenext. |     |     |     | Fromthepointofviewofauseror |     |     |
| ------------------------------- | --- | --- | --- | --------------------------- | --- | --- |
...
withmanyothervirtualmachines, onacomputerrunningadifferentoperating
system.
...
objectcontainsdataandprovidesmethodsthatoperateonthedata. Aprocess
| is an object | that | contains | the following | data: |     |     |
| ------------ | ---- | -------- | ------------- | ----- | --- | --- |

2.2. Isolation 11
(cid:136) The text of the program, usually a sequence of machine language in-
structions.
(cid:136) Data associated with the program, including static data (allocated at
|     | compile | time) | and dynamic |     | data (allocated | at run time). |
| --- | ------- | ----- | ----------- | --- | --------------- | ------------- |
(cid:136) The state of any pending input/output operations. For example, if the
...
(cid:136) The hardware state of the program, which includes data stored in regis-
...
| --------- | ---- | ---- | ------ | --- | --- | --- |
(cid:136) Multitasking: Most operating systems have the ability to interrupt a
...
| --- | ---------- | --- | -------------- | --- | --- | --- |
(cid:136) Virtual memory: Most operating systems create the illusion that each
...
up the shell. If I type make in the shell, it creates a new process to run
...
If you use the -e flag, you get every process (including processes belonging to
...
are done.
is a process the operating system uses to create new threads. We’ll
kthreadd
talk more about threads later, but for now you can think of a thread as kind of
a process. The k at the beginning stands for kernel, which is the part of the
operating system responsible for core capabilities like creating threads. The
extradattheendstandsfordaemon, whichisanothernameforprocesseslike
...
| ------------- | ---------- | ------------------ | --------- | -------------- | --- |
kworkerisaworkerprocesscreatedbythekerneltodosomekindofprocessing
for the kernel.

| 14  |     |     |     | Chapter | 2. Processes |
| --- | --- | --- | --- | ------- | ------------ |
Thereareoftenmultipleprocessesrunningthesekernelservices. Onmysystem
...
you can search for more information about them. You should run ps on your
...
| -------- | -------------- | --- | --- | --- |
Goingintheotherdirection, supposeyouwanttostorealetterofthealphabet.
...
log 6 bits of information. And in general, if the probability of the outcome is
2
| 1 in N, then | the outcome | contains | log N bits | of information. |
| ------------ | ----------- | -------- | ---------- | --------------- |
2
Equivalently, if the probability of the outcome is p, then the information con-
tent is −log p. This quantity is called the self-information of the outcome.
2
It measures how surprising the outcome is, which is why it is also called sur-
prisal. If your horse has only one chance in 16 of winning, and he wins, you
...
and forth between the number of bits, b, and the number of values they can
encode, N = 2b.
...
puters, main memory is volatile, which means that when the computer shuts
...
set of valid physical addresses is called the physical address space. It usually
runs from 0 to N − 1, where N is the size of main memory. On a system
with 1 GiB of physical memory, the highest valid address is 230 −1, which is
...
to M −1, where M is the number of valid virtual addresses. The size of the
...
termsindicatethesizeoftheregisters, whichisusuallyalsothesizeofavirtual
...
232
| space | is  | bytes, | or  | 4 GiB. |     |     |     |     |
| ----- | --- | ------ | --- | ------ | --- | --- | --- | --- |
Ona64-bitsystem,thesizeofthevirtualaddressspaceis264 bytes,or24·10246
...
(cid:136) The code segment contains the program text; that is, the machine
...
| --- | ---------- | --- | ---- | --------- | --- | ---------- | -------- | --- |
(cid:136) The global segment contains global variables and local variables that
|     | are | declared | static. |     |     |     |     |     |
| --- | --- | -------- | ------- | --- | --- | --- | --- | --- |

| 18  |     |     |     |     |     |     | Chapter | 3. Virtual | memory |
| --- | --- | --- | --- | --- | --- | --- | ------- | ---------- | ------ |
(cid:136) The heap segment contains chunks of memory allocated at run time,
|     | most | often | by calling | the C | library | function | malloc. |     |     |
| --- | ---- | ----- | ---------- | ----- | ------- | -------- | ------- | --- | --- |
(cid:136) The stack segment contains the call stack, which is a sequence of stack
...
|     | near | 0.  |     |     |     |     |     |     |     |
| --- | ---- | --- | --- | --- | --- | --- | --- | --- | --- |
Thestaticsegmentisoftenjustabovethetextsegment, thatis, athigher
(cid:136)
addresses.
|     | The | global | segment | is often | just | above | the static | segment. |     |
| --- | --- | ------ | ------- | -------- | ---- | ----- | ---------- | -------- | --- |
(cid:136)
(cid:136) The heap is often above the global segment. As it expands, it grows up
|     | toward |     | larger addresses. |     |     |     |     |     |     |
| --- | ------ | --- | ----------------- | --- | --- | --- | --- | --- | --- |
(cid:136) The stack is near the top of memory; that is, near the highest addresses
...
program, whichisinaspace.cintherepositoryforthisbook(seeSection0.1).
| #include |     | <stdio.h>  |     |     |     |     |     |     |     |
| -------- | --- | ---------- | --- | --- | --- | --- | --- | --- | --- |
| #include |     | <stdlib.h> |     |     |     |     |     |     |     |
int global;
...
address of the first machine language instruction in main, which we expect to
| be in the | text | segment. |     |     |     |     |
| --------- | ---- | -------- | --- | --- | --- | --- |
global is a global variable, so we expect it to be in the global segment. local
| is a local | variable, | so  | we expect | it to be | on the stack. |     |
| ---------- | --------- | --- | --------- | -------- | ------------- | --- |
s refers to a “string literal”, which is a string that appears as part of the
...
| to the pointer, |     | s, which | is a | local variable). |     |     |
| --------------- | --- | -------- | ---- | ---------------- | --- | --- |
...
As expected, the address of is the lowest, followed by the location of the
main
string literal. The location of is next, then the address points to.
global p
...
| virtual | address | space | is 248 bytes. |     |     |     |
| ------- | ------- | ----- | ------------- | --- | --- | --- |
As an exercise, run this program on your computer and compare your results
to mine. Add a second call to malloc and check whether the heap on your
...
| -------- | -------- | -------- | --- | --- | --- | --- | --- | --- | --- |
In C there is another kind of local variable, called static, which is allocated
...
| -------------- | ------- | --- | ---- | ----- | -------- | ------- | ------- | --- | --- |
Ifyouaddthisfunctiontoaspace.cyoucanconfirmthatcounterisallocated
...
| ------------ | ----------- | --- | --- | --- | --- | --- | --- |
Mostprocessorsprovideamemorymanagementunit(MMU)thatsitsbetween
...
| ------ | --------- | ---------------- | --- | --- | --- | --- | --- |
(cid:136) The size of the virtual address space is 232 B and the size of a page is
| 210 B, | so there are | 222 virtual | pages. |     |     |     |     |
| ------ | ------------ | ----------- | ------ | --- | --- | --- | --- |
(cid:136) The size of the offset is determined by the page size. In this example the
| page size | is 210 B, | so it takes | 10 bits | to specify | a byte | on a page. |     |
| --------- | --------- | ----------- | ------- | ---------- | ------ | ---------- | --- |
(cid:136) If a VA is 32 bits and the offset is 10 bits, the remaining 22 bits make
| up the | virtual page | number. |     |     |     |     |     |
| ------ | ------------ | ------- | --- | --- | --- | --- | --- |
220
(cid:136) Since there are physical pages, each physical page number is 20 bits.
...
frame. So we expect 3–4 bytes per entry. But with 222 virtual pages, the page
table would require 224 bytes, or 16 MiB.
Andsinceweneedapagetableforeachprocess,asystemrunning256processes
wouldneed232 bytes, or4GiB,justforpagetables! Andthat’sjustwith32-bit
...
org/wiki/Page_table; you might find the details interesting. But the fun-
...
| ------ | ----------- | --- | --------- | ---------------- | --- |
Abstractly:
(cid:136) A “file system” is a mapping from each file’s name to its contents. If
...
Key-value_database).
| (cid:136) A “file” |     | is a sequence |     | of bytes. |     |
| ------------------ | --- | ------------- | --- | --------- | --- |
...
Theprimarydifferencebetweentheabstractionandtheunderlyingmechanism
isthatfilesarebyte-basedandpersistentstorageisblock-based. Theoperating
...
2. Itfindsthefilenamedfile.txtand“opens”itforreading, whichmeans
...
3. When we call fgetc, the operating system checks whether the next char-
...
6. When the process closes the file, the operating system completes or can-
...
/home/downey.
2. TheoperatingsystemcreatesanOpenFileTableEntrythatindicatesthat
...
ms(seehttps://en.wikipedia.org/wiki/Hard_disk_drive_performance_
characteristics). SSDs are faster, taking 25 µs to read a 4 KiB block and
250 µs to write one (see http://en.wikipedia.org/wiki/Ssd#Controller).
...
(cid:136) Block transfers: The time it takes to load a single byte from disk is 5–
...
access the disk.
(cid:136) Prefetching: Sometimes the operating system can predict that a process
...
(cid:136) Buffering: As I mentioned, when you write a file, the operating system
...
(cid:136) Caching: If a process has used a block recently, it is likely to use it again
...
them,withtwoexceptions: (1)iftheperformanceofaprogramisunexpectedly
...
4.2. Disk metadata 27
4.2 Disk metadata
Theblocksthatmakeupafilemightbearrangedcontiguouslyondisk, andfile
...
Iftheblocksizeis8KiB,thefirst12blocksmakeup96KiB.Onmostsystems,
...
a linked list. For more details, see http://en.wikipedia.org/wiki/File_
Allocation_Table.
...
(cid:136) Speed: Allocating and freeing blocks should be fast.
(cid:136) Minimal space overhead: The data structures used by the allocator
should be small, leaving as much space as possible for data.
(cid:136) Minimal fragmentation: If some blocks are left unused, or some are only
partially used, the unused space is called “fragmentation”.
(cid:136) Maximum contiguity: Data that is likely to be used at the same time
...
UNIXfilesystem, toext3, a“journaling”filesystemintendedtoimprovespeed
...
file open for writing, so it can use C library functions like fputs and fprintf.
...
| for example, | the | representation |     | for 5 | is b101. |
| ------------ | --- | -------------- | --- | ----- | -------- |
10
...
| --------- | ------- | ---- | ------------ | --- | --- |
To find the two’s complement of a negative number, −x, find the binary rep-
resentation of x, flip all the bits, and add 1. For example, to represent −5 ,
10
start with the representation of 5 , which is b00000101 if we write the 8-bit
10
| version. | Flipping | all the | bits | and adding | 1 yields b11111011. |
| -------- | -------- | ------- | ---- | ---------- | ------------------- |
...
bers) unless you declare them unsigned. The difference, and the reason this
...
are 1, and 0 otherwise. Here is an example of & applied to two 4-bit numbers:
1100
& 1010
----
1000
In C, this means that the expression 12 & 10 has the value 8.
Similarly, | computes the OR operation, which yields 1 if either operand is 1,
and 0 otherwise.
1100
| 1010
----
...
| m/s is | approximately | 2.998·108. |     |     |     |     |     |
| ------ | ------------- | ---------- | --- | --- | --- | --- | --- |
...
example, in base 10, we prefer 2.998·108 rather than 2998·105 or any other
...
| --- | --- | --- | --- | --- | --- | ------- | ------- | -------- | ----- |
For example, the integer representation of 13 is b1101. In floating point,
10
that’s 1.101·23, so the exponent is 3 and the part of the coefficient that would
...
The union allows us to store a floating-point value using p.f and then read it
| as an unsigned |     | integer | using | p.u. |     |     |     |     |     |
| -------------- | --- | ------- | ----- | ---- | --- | --- | --- | --- | --- |
...
| -------- | ----- | ------ | --- | --- | --- | --- | --- | --- | --- |
Asanexercise, tryassemblingordisassemblingadouble, whichusesthe64-bit
standard. See http://en.wikipedia.org/wiki/IEEE_floating_point.
...
NextI’lldefineafunctionthatcreatesasmallerarrayanddeliberatelyaccesses
...
98
99
Thedetailsheredependonthecompiler, whicharrangesvariablesonthestack.
From these results, we can infer that the compiler put x and y next to each
...
array, the values you read might have any type. For example, if I change f1
...
Relatedissuessometimescomeupwithstrings. First, rememberthatCstrings
...
(cid:136) malloc, which takes an integer size, in bytes, and returns a pointer to
...
(cid:136) calloc, which is the same as malloc except that it also clears the newly
| allocated | chunk; | that is, | it sets all bytes | in the chunk | to 0. |
| --------- | ------ | -------- | ----------------- | ------------ | ----- |
(cid:136) free, which takes a pointer to a previously allocated chunk and deallo-
cates it; that is, it makes the space available for future allocation.
realloc, which takes a pointer to a previously allocated chunk and a
(cid:136)
...
ter on the animated television program The Simpsons; in a few episodes, he
...
(cid:136) If you access (read or write) any chunk that has not been allocated,
that’s a paddling.
(cid:136) If you free an allocated chunk and then access it, that’s a paddling.
(cid:136) If you try to free a chunk that has not been allocated, that’s a paddling.
(cid:136) If you free the same chunk more than once, that’s a paddling.
(cid:136) If you call realloc with a chunk that was not allocated, or was allocated
and then freed, that’s a paddling.
Itmightnotsounddifficulttofollowtheserules,butinalargeprogramachunk
...
(cid:136) If you read a value from an unallocated chunk, the system might detect
...
(cid:136) If you write a value to an unallocated chunk, and don’t get a segmenta-
...
ment malloc and free (which we will see soon) are often stored along with the
...
usually won’t detect the problem until later, when you call malloc or free,
...
(cid:136) At some point, the system runs out of physical memory. On systems
without virtual memory, the next call to malloc will fail, returning
NULL.

| 42  |     |     |     | Chapter | 6. Memory | management |
| --- | --- | --- | --- | ------- | --------- | ---------- |
(cid:136) Onsystemswithvirtualmemory, theoperatingsystemcanmoveanother
...
| ------- | -------- | --------- | ---- | --------- | ---------- | ---- |
(cid:136) There might be a limit on the amount of space a single process can
| allocate; | beyond | that, | malloc | returns NULL. |     |     |
| --------- | ------ | ----- | ------ | ------------- | --- | --- |
(cid:136) Eventually, a process might fill its virtual address space (or the usable
...
style to check the result from malloc before using it. One option is to add a
| condition      | like this       | after every | malloc | call: |     |     |
| -------------- | --------------- | ----------- | ------ | ----- | --- | --- |
| void *p        | = malloc(size); |             |        |       |     |     |
| if (p          | == NULL)        | {           |        |       |     |     |
...
is declared in stdio.h; it prints an error message and additional in-
perror
...
| most memory, | you can        | use the UNIX | utilities | ps and top. |
| ------------ | -------------- | ------------ | --------- | ----------- |
| 6.3          | Implementation |              |           |             |
...
Theruntimeofmallocdoesnotusuallydependonthesizeofthechunk,
(cid:136)
but might depend on how many free chunks there are. free is usually
fast, regardless of the number of free chunks. Because calloc clears
...
| --- | --- | ------- | --------- | ---------- |
(cid:136) Boundary tags: When malloc allocates a chunk, it adds space at the
...
| ------------- | ------------- | --------------- | ----- | --- |
(cid:136) Space overhead: Boundary tags and free list pointers take up space.
The minimum chunk size on most systems is 16 bytes. So for very small
chunks, is not space efficient. If your program requires large
malloc
numbers of small structures, it might be more efficient to allocate them
in arrays.
(cid:136) Fragmentation: If you allocate and free chunks with varied sizes, the
...
| ----------- | ------- | ------------- | ----------- | ---------- |
(cid:136) Binning and caching: The free list is sorted by size into bins, so when
...
| ------- | -------- | --- | --- |
Inordertounderstandcaching,youhavetounderstandhowcomputersexecute
...
| --------------- | -------- | --- | --- |
(cid:136) The program counter, or PC, which contains the address (in memory)
| of the | next instruction | in  | the program. |
| ------ | ---------------- | --- | ------------ |
(cid:136) The instruction register, or IR, which contains the machine code instruc-
| tion | currently | executing. |     |
| ---- | --------- | ---------- | --- |
(cid:136) The stack pointer, or SP, which contains the address of the stack frame
for the current function, which contains its parameters and local vari-
ables.
(cid:136) General-purpose registers that hold the data the program is currently
| working | with. |     |     |
| ------- | ----- | --- | --- |

| 46  |     |     |     |     |     |     |     | Chapter | 7. Caching |
| --- | --- | --- | --- | --- | --- | --- | --- | ------- | ---------- |
(cid:136) A status register, or flag register, that contains information about the
...
| “instruction |     |     | cycle”: |     |     |     |     |     |     |
| ------------ | --- | --- | ------- | --- | --- | --- | --- | --- | --- |
(cid:136) Fetch: The next instruction is fetched from memory and stored in the
|     | instruction |     | register. |     |     |     |     |     |     |
| --- | ----------- | --- | --------- | --- | --- | --- | --- | --- | --- |
(cid:136) Decode: Part of the CPU, called the “control unit”, decodes the instruc-
|     | tion | and | sends | signals | to the other | parts | of the | CPU. |     |
| --- | ---- | --- | ----- | ------- | ------------ | ----- | ------ | ---- | --- |
(cid:136) Execute: Signals from the control unit cause the appropriate computa-
|     | tion | to  | occur. |     |     |     |     |     |     |
| --- | ---- | --- | ------ | --- | --- | --- | --- | --- | --- |
...
| --- | ---------------- | ---------- | --- | ---------- | ---------- | -------- | ----------- | --- | --- |
|     | (cid:136) Store: | Transfers  |     | a value    | from a     | register | to memory.  |     |     |
(cid:136) Jump/branch: Changes the program counter, causing the flow of execu-
...
| ------- | --- | ------ | --- | ------------- | ---------- | --- | --- | --- | --- |
Duringeachinstructioncycle,oneinstructionisreadfromtheprogramtext. In
...
to memory. If the time to process a cache hit is T and the time for a cache
h
| miss is | T , the average | time for each | memory | access | is  |     |
| ------- | --------------- | ------------- | ------ | ------ | --- | --- |
m
hT +mT
h m
Equivalently, we could define the “miss penalty” as the extra time to process
| a cache | miss, T = T | −T . Then | the average | access | time is |     |
| ------- | ----------- | --------- | ----------- | ------ | ------- | --- |
|         | p           | m h       |             |        |         |     |
T +mT
h p
When the miss rate is low, the average access time can be close to T . That
h
...
of locality:
(cid:136) Mostprogramscontainblocksofcodewithnojumpsorbranches. Within
these blocks, instructions run sequentially, so the access pattern has
| spatial | locality. |     |     |     |     |
| ------- | --------- | --- | --- | --- | --- |
(cid:136) In a loop, programs execute the same instructions many times, so the
| access | pattern | has temporal | locality. |     |     |
| ------ | ------- | ------------ | --------- | --- | --- |
(cid:136) The result of one instruction is often used immediately as an operand of
...
ity.
(cid:136) One of the most common processing patterns is to read or write the
...
possibletoinferthesizeofthecache, theblocksize, andsomeotherattributes.
My modified version of this program is in the cache directory of the repository
...
example, if limit is 16 and stride is 4, the loop would access elements 0, 4,
8, and 12.
...
4
2
|     |     |     | 0 212 | 214 216 | 218 | 220 222 | 224 | 226 |     |
| --- | --- | --- | ----- | ------- | --- | ------- | --- | --- | --- |
size (B)
...
sec contains the total time for all array accesses, minus the total time it took
to increment temp. This difference is the total miss penalty incurred by all
...
| 7.5. | Programming |     |     | for | cache | performance |     | 51  |
| ---- | ----------- | --- | --- | --- | ----- | ----------- | --- | --- |
(cid:136) The program reads through the array many times, so it has plenty of
temporal locality. If the entire array fits in cache, we expect the average
|     | miss | penalty |     | to be | near | 0.  |     |     |
| --- | ---- | ------- | --- | ----- | ---- | --- | --- | --- |
(cid:136) When the stride is 4 bytes, we read every element of the array, so the
...
the cache size or if the stride is smaller than the block size. Performance only
...
expensive. Mainmemoryisusuallydynamicrandom-accessmemory(DRAM),
whichusesonlyonetransistorandonecapacitorperbit,soitispossibletopack
...
caching:
(cid:136) Whomovesdataupanddownthehierarchy? Atthetopofthehierarchy,
registerallocationisusuallydonebythecompiler. HardwareontheCPU
...
thebottomofthehierarchy, administratorsmovedataexplicitlybetween
disk and tape.
(cid:136) What gets moved? In general, block sizes are small at the top of the
...
time.
(cid:136) When does data get moved? In the most basic cache, data gets moved
...
(cid:136) Where in the cache does the data go? When the cache is full, we can’t
...
not been used recently. For more on this topic, see http://en.wikipedia.
...
(cid:136) Most processes don’t use all of their allocated memory. Many parts of
...
(cid:136)
neveraccessitagain. Byswappingthosepagesout, theoperatingsystem
| can effectively | plug | the leak. |     |     |     |     |
| --------------- | ---- | --------- | --- | --- | --- | --- |
(cid:136) On most systems, there are processes like daemons that sit idle most of
...
| --- | --- | --- | ------- | ---------- |
(cid:136) A user might have many windows open, but only a few are active at a
| time. The | inactive processes | can be swapped | out. |     |
| --------- | ------------------ | -------------- | ---- | --- |
(cid:136) Also, there might be many processes running the same program. These
...
“multitasking”, whichmeansitcanswitchfromoneprocesstoanotherquickly,
creating the illusion that many processes are running at the same time.
Thepartoftheoperatingsystemthatimplementsmultitaskingisthe“kernel”.
...
severalotherlayers,includinganinterfacecalleda“shell.” Computerscientists
...
3. The interrupt handler runs whatever code is needed to handle the inter-
rupt.
...
is one of:
|     | (cid:136) Running, |     | if the | process | is  | currently | running | on a core. |     |
| --- | ------------------ | --- | ------ | ------- | --- | --------- | ------- | ---------- | --- |
...
| --- | --- | --- | ---- | --------- | --- | --- | --- | --- | --- |
Herearetheeventsthatcauseaprocesstotransitionfromonestatetoanother:
(cid:136) A process is created when the running program executes a system call
...
| --- | --- | --- | --- | --- | --- | --- | --- | ------- | --------------- |
(cid:136) When a process is started or resumed by the scheduler, its state changes
|     | from | ready | to running. |     |     |     |     |     |     |
| --- | ---- | ----- | ----------- | --- | --- | --- | --- | --- | --- |
(cid:136) When a process is interrupted and the scheduler chooses not to let it
|     | resume, |     | its state | changes | from | running | to  | ready. |     |
| --- | ------- | --- | --------- | ------- | ---- | ------- | --- | ------ | --- |
(cid:136) If a process executes a system call that cannot complete immediately,
like a disk request, it becomes blocked and the scheduler usually chooses
|     | another |     | process. |     |     |     |     |     |     |
| --- | ------- | --- | -------- | --- | --- | --- | --- | --- | --- |
(cid:136) When an operation like a disk request completes, it causes an interrupt.
...
| --- | --- | --- | ------- | ------ | --- | ---------- | --------- | --- | -------- |
(cid:136) When a process calls exit, the interrupt handler stores the exit code in
...
| --- | ------ | --- | ------------- | --- | --------- | ---- | --------------- | --- | --- |
(cid:136) Processes might be limited by different resources. A process that does
...
time-sensitive.
(cid:136) If a process is likely to run for a short time and then make a blocking
...
Mostschedulersusesomeformofpriority-basedscheduling,whereeachprocess
...
(cid:136) Aprocessusuallystartswitharelativelyhighprioritysoitstartsrunning
quickly.
(cid:136) If a process makes a request and blocks before its time slice is complete,
it is more likely to be interactive or I/O-bound, so its priority should go
up.
(cid:136) If a process runs for an entire time slice, it is more likely to be long-
running and CPU-bound, so its priority should go down.
(cid:136) If a task blocks for a long time and then becomes ready, it should get a
priority boost so it can respond to whatever it was waiting for.
(cid:136) If process A is blocked waiting for process B, for example if they are
connected by a pipe, the priority of process B should go up.
(cid:136) The system call nice allows a process to decrease (but not increase) its
...
quencyandreacttoexternaleventswithsomemaximumresponsetime. These
...
| (cid:136) Providing | richer | APIs for controlling |     | task priorities. |     |
| ------------------- | ------ | -------------------- | --- | ---------------- | --- |
(cid:136) Modifying the scheduler to guarantee that the process with highest pri-
...
(cid:136)
thenextchapter)toallowahigh-prioritytasktopreemptalower-priority
task.
(cid:136) Choosing an implementation of dynamic memory allocation that guar-
...
| -------- | ---------- | --------------- | --- | --- |
Theprocesseswehaveseensofarare“single-threaded”, whichmeansthatonly
...
feringwitheachother. Usuallythreadsdon’taccesseachother’slocalvariables
...
| (cid:136) You     | include  | headers |           | files at the    | beginning  | of  | your program. |     |
| (cid:136) You     | write    | code    | that      | calls functions | defined    | by  | Pthreads.     |     |
(cid:136) When you compile the program, you link it with the Pthread library.
...
This compiles a source file named array.c with debugging info and opti-
...
| -------- | ----------- | --- | ----------- | ------- | --- | --- |
If pthread create succeeds, it returns 0 and make_thread returns the handle
of the new thread. If an error occurs, pthread create returns an error code
| and make_thread |     | prints | an error | message | and exits. |     |
| --------------- | --- | ------ | -------- | ------- | ---------- | --- |
The parameters of make_thread take some explaining. Starting with the sec-
ond, Shared is a structure I defined to contain values shared between threads.
| The following |          | typedef | statement | creates the | new type: |     |
| ------------- | -------- | ------- | --------- | ----------- | --------- | --- |
| typedef       | struct   | {       |           |             |           |     |
| int           | counter; |         |           |             |           |     |
} Shared;
In this case, the only shared variable is counter. make shared allocates space
| for a Shared |                | structure | and initializes | the contents: |     |     |
| ------------ | -------------- | --------- | --------------- | ------------- | --- | --- |
| Shared       | *make_shared() |           |                 |               |     |     |
{
...
| wrapper | does | is call | pthread | join | and | check | the | result. |     |
| ------- | ---- | ------- | ------- | ---- | --- | ----- | --- | ------- | --- |
...
thread to read a different value of counter and increment it, so that the value
of counter reflects the number of threads that have executed child_code.
...
| -------- | ------- | ------- | ----------- | -------- | --- | --- |
| 9.5      | Mutex   |         |             |          |     |     |
My definition of Mutex is a wrapper for a type called pthread_mutex_t, which
...
a circular buffer, which you can read about at https://en.wikipedia.org/
...
theelementsareints, butmoregenerallytheywouldbestructuresthatcontain
| user events, | items | of work, | etc. |     |     |     |
| ------------ | ----- | -------- | ---- | --- | --- | --- |
length is the length of the array. next_in is an index into the array that
indices where the next element should be added; similarly, is the
next_out
...
(cid:136) Access to the queue is not thread safe. Different threads could access
...
in queue_mutex.c.
| First | we  | add | a   | pointer | to the queue | structure: |     |     |
| ----- | --- | --- | --- | ------- | ------------ | ---------- | --- | --- |
Mutex
| typedef |       | struct    | {   |     |      |           |        |     |
| ------- | ----- | --------- | --- | --- | ---- | --------- | ------ | --- |
...
queue_incrdonottrytolockthemutex. Anythreadthatcallsthesefunctions
...
threads to block until the condition becomes true. For example, thread_pop
...
| ----------- | --- | ------ | ------------- | ----- | ------ | --------- | --- |
So while the consumer is blocked on nonempty, the producer can run. Let’s
...
full. Assuming it is not, queue_push adds a new element to the queue and
| then unlocks | the | Mutex. |     |     |     |     |     |
| ------------ | --- | ------ | --- | --- | --- | --- | --- |
Butbeforereturning,itdoesonemorething: it“signals”theconditionvariable
nonempty.
...
blocked and resumes execution of cond_wait. But before the awakened thread
can return from cond_wait, it has to wait for and lock the Mutex, again.
...
condition is true, so there is at least one item in the queue, and (2) the Mutex
...
(cid:136) Why is cond_wait inside a while loop rather than an if statement; that
...
of an intercepted signal. Suppose Thread A is waiting on nonempty.
...
cond_wait. If Thread A does not check the condition again, it would try
to pop an element from an empty queue, and probably cause an error.
(cid:136) The other question that comes up when people learn about condition
...
between a Cond structure and the condition it relates to. The connection
is implicit in the way it is used.
Here’s one way to think of it: the condition associated with a Cond is
the thing that is false when you call cond_wait and true when you call
cond_signal.
Becausethreadshavetochecktheconditionwhentheyreturnfromcond_wait,
...
similar to Mutex, which is a wrapper for pthread_mutex_t. Both wrappers
| are defined | in             | utils.c | and utils.h. |     |     |     |     |
| ----------- | -------------- | ------- | ------------ | --- | --- | --- | --- |
| Here’s      | the typedef:   |         |              |     |     |     |     |
...
Pthreads,butmostUNIXesthatimplementPthreadsalsoprovidesemaphores.
...
| --- | --- | --- | --- | --- | ------- | -------------- | ---- |
Semaphore is a synonym for sem_t, but I find it more readable, and the capital
...
Semaphore.
sem init returns 0 if it succeeds and -1 if anything goes wrong. One nice thing
...
thatthemutexisunlocked; thatis, onethreadcanpassthesemaphorewithout
blocking.
...
in queue_sem.c.
Here’sthenewdefinitionofQueue, replacingthemutexandconditionvariables
with semaphores:
| typedef   | struct    | {        |     |      |     |     |     |
| --------- | --------- | -------- | --- | ---- | --- | --- | --- |
| int       | *array;   |          |     |      |     |     |     |
...
| --- | --- | --- | ------- | -------------- | ---- |
mutex is used to guarantee exclusive access to the queue; the initial value is 1,
| so the mutex | is initially | unlocked. |     |     |     |
| ------------ | ------------ | --------- | --- | --- | --- |
items is the number of items in the queue, which is also the number of con-
sumer threads that can execute queue_pop without blocking. Initially there
...
number of spaces is the capacity of the queue, which is length-1, as explained
| in Section | 10.1. |     |     |     |     |
| ---------- | ----- | --- | --- | --- | --- |
...
tions that implement the semaphore API in sem.h using using condition vari-
...
HereismyimplementationofsemaphoresusingPOSIXmutexesandcondition
variables:
...
blocksuntila“wakeup”isavailable. Whileitisblocked,themutexisunlocked,
...
variable again. If so, it decrements wakeups, unlocks the mutex, and exits.
